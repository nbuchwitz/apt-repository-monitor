...
def new_package_notification(
    ...
    text = (
        f"Es wurden {len(packages)} neue Pakete im APT-Repository {repository} veröffentlicht.\n"
        "Die Liste aller Änderungen findet sich unter "
        "https://kunbus-gmbh.atlassian.net/wiki/spaces/EN/pages/3264774241/Rolling+Release+Notes"
    )
    facts = [{"name": package.name, "value": package.version} for package in packages]
...
def notification(webhook_url: str, title: str, text: str, facts: list):
    response = requests.post(
        ...
        json={
            "themeColor": "#ff9900",
            "summary": title,
            "sections": [{"activityTitle": title, "activitySubtitle": text, "facts": facts}],
        },
    )
...
if __name__ == "__main__":
    ...
    try:
        for repo in repos:
            ...
            if new_packages:
                rc = new_package_notification(webhook_url, repo.url, repo.dist, repo.arch, new_packages)
    except Exception as exc:
